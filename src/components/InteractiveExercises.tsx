...
import { Wind, Anchor, Brain, Moon, X, Check, ArrowRight, Heart, Sparkles, MessageSquare } from 'lucide-react';
    ...
    <section id="exercicios" className="py-20 bg-[#1A1A1A] text-[#FDFCFB] border-b border-[#333]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h2 className="font-display font-bold text-3xl sm:text-4xl text-[#FDFCFB] tracking-tight mb-3">
            Exercícios Guiados para Lidar com Emoções Difíceis
          </h2>
          <p className="font-sans text-xs text-[#8E8A83]">
            Ferramentas interativas de Terapia Cognitivo-Comportamental para acalmar a mente e focar no presente.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {EXERCISES.map((ex, idx) => {
            const Icon = ex.icon;
            return (
              <div key={ex.id} className="bg-[#242424] border border-[#333] hover:border-[#C5A059] p-5 flex flex-col justify-between transition-colors group">
                <div>
                  ...
                  <p className="font-sans text-[11px] text-[#C5A059] font-semibold mb-2">{ex.subtitle}</p>
                  <p className="font-sans text-xs text-[#8E8A83] leading-relaxed mb-5">{ex.desc}</p>
                </div>
                <button
                  onClick={() => openEx(idx)}
                  className="w-full py-2 bg-[#1A1A1A] hover:bg-[#C5A059] hover:text-[#1A1A1A] border border-[#333] hover:border-[#C5A059] text-[11px] font-bold uppercase tracking-wider transition-colors flex items-center justify-center space-x-1.5 cursor-pointer"
                >
