...
export default function ArticlesPage() {
  ...
  return <main className="min-h-screen bg-[#F9F7F2] pt-32 pb-24"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {!loading && posts.length > 0 && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{posts.map((post, idx) => {
      const imageUrl = sanityImageUrl(post?.coverImage);
      const slugStr = typeof post?.slug === 'string' ? post.slug : ((post?.slug as any)?.current || '');
      const articleLink = slugStr ? `/artigos/${slugStr}` : '/artigos';
      return <article key={post?._id || idx} className="flex flex-col overflow-hidden bg-[#FDFCFB] border border-[#E5E1DA] hover:border-[#1A1A1A] editorial-shadow transition-colors">
        {imageUrl && <img src={imageUrl} alt="" className="aspect-[16/10] w-full object-cover" />}
        <div className="p-6 flex flex-col flex-1"><p className="flex items-center text-[10px] font-mono font-bold text-[#8E8A83] mb-3"><Calendar className="w-3 h-3 mr-1" />{formatDate(post?.publishedAt)}</p><h2 className="font-display font-bold text-xl text-[#1A1A1A] leading-snug mb-3">{post?.title || 'Sem título'}</h2>{post?.excerpt && <p className="font-sans text-xs text-[#2C3531] leading-relaxed mb-6">{post.excerpt}</p>}<Link to={articleLink} className="mt-auto inline-flex items-center text-[10px] font-bold uppercase tracking-wider text-[#1A1A1A] hover:text-[#8E8A83]">Ler artigo <ArrowRight className="w-3.5 h-3.5 ml-1.5" /></Link></div>
      </article>;
    })}</div>}
  </div></main>;
}
